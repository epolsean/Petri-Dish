...
// CoroutineManager is just a proxy object so we have a launcher for the coroutines.
public class CoroutineManager : MonoBehaviour 
{
    // only one CoroutineManager can exist. We use a singleton pattern to enforce this.
    ...
    public static CoroutineManager instance
    {
        get
        {
            if(!_instance)
            {
                // check if a CoroutineManager is already available in the scene graph
    ...
    private IEnumerator doWork()
    {
        ...
        while(_running)
        {
            ...
            else
            {
                ...
                {
                    // run out child jobs if we have any
                    if (_childJobStack != null)
